...
room bolted onto a silo — a **bay** that lets one more of its own workers in — and the **machinery**
and **polymers** it fits to a zone, which everyone then works with, and pays the owner to use.
...
- **Machinery** and **polymers** are not goods you hold. They are permanent upgrades placed on the
  board (see Upgrades). So are **bays**.
...
Upgrades (one zone each, 1 tick, all payable with time for a second — see Paying with time):
...
**Every zone on the board is 1 tick.** The upgrade zones were 2 for one design pass, so that
upgrades waited longer than goods; that made the one-zone silos sluggish on paper, and paying with
time already adds the tick for the player who wants the discount. The only 2-tick zone now is one
someone chose to make so.
...
Beside every silo runs a **time marker** track. Beside every zone is an empty **bay** square,
unusable until someone builds it (see Bays).
...
| D1   | Make Machinery — metal + energy      | 1     | 1 (+1 with time)   |
| D2   | Make Polymers — chemical + energy    | 1     | 1 (+1 with time)   |
| D3   | Recruit worker — food + energy       | 1     | 1 (+1 with time)   |
| E1   | Construction — rock + metal + energy | 1     | 1 (+1 with time)   |
| E2   | Specialist — water + food + energy   | 1     | 1 (+1 with time)   |
...
Every zone has room for one extra worker slot, the **bay**, printed as an empty square on the zone's
right side. It does nothing until a player **builds** it via Construction (E1), which puts that
player's marker in the square.

- **A built bay is a second slot in its zone, for its owner only.** Nobody else may place there. A
  worker placed in a bay pays the zone's cost, waits the zone's ticks, is bumped with the zone and
  takes the zone's yield — identical to the base slot. It locks and unlocks with the zone.
- **No rent, no sharing.** A bay is private room: it lets its owner double up on a zone that would
  otherwise hold one worker. The return is the room itself, and whatever VP bays end up scoring. (An
  earlier draft let rivals rent a bay for 1 Energy; that toll now lives on machinery and polymers,
  which are shared, so a bay is the one owned thing with nothing to track.)
- **Lowest to highest.** Within a silo, bays must be built bottom-up: the first bay in Energy is on
  Energy I, the next on Energy II, and so on. A silo whose every zone has a bay cannot be chosen.
  This keeps the cheap, crowded zones the first to widen.
- **Capacity by player count.** At 2–3 players each zone holds one bay, which is what the board
  prints. At 4 players every zone should hold two; a second square is a print change for later.

Bays are also the answer to the single-zone D/E silos sealing shut: a bay on Machinery is a second
machinist's chair that only its owner can sit in, so the silo can no longer be closed to them.
...
- **Place** — put a worker from your supply into a silo. It goes into the **lowest unlocked zone
  whose base slot is empty**, or into any empty, unlocked **bay of yours** at or below that zone.
  You may never climb past an empty base slot; a bay is a choice, the base slot is not. **Pay that
  zone's cost immediately**, plus the **toll** for any machinery or polymer on the zone that is not
  yours (see Upgrades). Paying on placement is the fundamental idea of the game: the cost is sunk
  before you know when the yield arrives.
...
of your own workers may share a zone (base and bay) across turns, never in one.
...
- **Bumping.** When the marker moves **past** a zone — exits it upward — any worker in that zone is
  **bumped**: it returns to its owner's supply along with the zone's **yield**. A zone holding a
  time token needs an extra tick to be passed (see Paying with time), so a discounted machine
  genuinely takes longer than rock. Base slot and bay bump together.
...
- **Reset.** The tick that carries the marker past the **top zone** bumps that zone and immediately
  **returns the marker to the bottom**. Everything unlocks at once. There is no parked state — the
  last tick out is also the first tick of the next cycle. On the **raw-goods silos** — Energy, Rock,
  Water — the player who ticks the reset takes **1 Energy** from the supply: a small thank-you for
  reopening the deep, crowded silos everyone needs. The refining and upgrade silos pay nothing for a
  reset; the one-zone silos reset on every bump, and a free structure for ticking Construction would
  be neither small nor obvious.
...
- **Construction (E1)** — **1 tick.** `2 Rock + 1 Metal + 3 Energy`, or `1 Rock + 2 Energy` **+
  time**. On placement, name a **silo** that still has an unbuilt bay. On bump, put your marker in
  the **lowest unbuilt bay** of that silo — which may be higher than it was when you placed, if
  someone built there first. See Bays. Construction replaces the structure-card deck of the first
  draft; bays are the metal sink and, probably, the VP engine, but VP is deferred until the loop is
  fun.
- **Recruit (D3)** — **1 tick.** `2 Food + 2 Energy`, or pay with time, for a new worker on bump.
  Worker count is the main tuning lever for how often turns fall back to (C); starting count **TBD
  from playtest**.
- **Machinery (D1)** — **1 tick.** `2 Metal + 2 Energy`, or pay with time, for one machine. Choose a
  **single zone in a B silo** (Rock or Water) with a free machinery slot; when bumped, place your
  machinery marker in it. The zone is now machined for **everyone**: every worker placed there gets
  the bonus yield. Every B zone prints **two** slots, so at most two players can machine one zone.
  Bonus size TBD.
- **Polymers (D2)** — **1 tick.** `2 Chemical + 2 Energy`, or pay with time, for one polymer. Same
  shape as machinery, targeting a **single zone in a C silo** (Metal, Chemical or Food): every
  worker there converts better. Two slots per C zone. Bonus size TBD.
- **Toll.** Placing into a machined or polymered zone, you pay **1 Energy to the owner of each
  upgrade there that is not yours**, on top of the zone's cost, on placement. No energy? Pay **1 of
  any good** instead. Nothing at all? You cannot place there. The bonus is not optional and neither
  is the toll — there is nothing to track beyond whose marker sits in the slot. A player who
  upgrades Rock I has turned the game's free zone into a toll booth, and that is the point: the
  cheapest, most crowded zones are where an upgrade pays back fastest.
- **Nothing else takes an upgrade.** Energy and the D/E silos have no machinery or polymer slots.
  Bays are the only thing to own there.
- **Specialist (E2)** — **1 tick.** `2 Water + 1 Food + 3 Energy`, or `1 Water + 2 Energy` **+
...
- **Silo board** with tick tracks, one time marker per silo, and a bay square beside every zone.
- **Bay markers** per player, placed in a bay square when Construction resolves.
...
- **Player count:** TBD; the diversification rule and small silos suggest 2–4. The board as printed
  is for 2–3 (one bay per zone).
- **End trigger and scoring:** **TBD, deliberately.** The first job is a loop that is fun to run;
  scoring is tuned after that. Candidates: bays (flat, or by the height of the zone they widen),
  machinery/polymer placements, leftover goods by value.
...
- **Closed-silo deadlock.** A silo with its marker high and its upper zones full is sealed until
  ...
  — and whether bays on the D/E silos relieve it.
- **Advancing as gift.** Ticking is tempo and often helps a rival more than you. Watch whether
  ...
  game to place-only until workers run out — and whether 1 Energy for a raw-silo reset is enough to
...
- **Worker starvation.** Two atoms a turn with workers stranded in silos means (C) turns are forced,
  ...
  now too easy.
- **Deferred upgrades.** Machinery, polymers, bays and specialists all take effect on bump, so an
...
- **Bay throughput.** A bay doubles a zone's output per cycle for one player. Watch whether widening
  Energy I and Rock I floods the raw economy, and whether anyone ever builds past the bottom zone.
- **Toll vs skip.** A rival's machine on Rock I means Rock is no longer free. Watch whether
  followers pay the toll, climb past to an unmachined zone, or stop visiting the silo — and whether
  a broke player being shut out of the free zones ever decides a game.
- **Toll as income.** Two machines on Rock I collect 2 Energy from every visitor. Watch whether the
  first machine is simply correct, whether the second slot is ever worth taking, and whether a toll
  on the bonus zone is a fair price for the bonus or a tax on players who never wanted it.
...
- **Energy glut.** Energy is cheap to make and paid out by the fistful. Watch whether it ever
  ...
  — and the 1-energy toll — are decoration.
...
- **2026-09-09, design pass (no play).** Structure cards dropped; Construction now builds bays.
...
- **2026-09-09, second pass (no play).** "Annex" renamed to **bay**. Machinery and polymers now
  boost every worker on their zone, and anyone but the owner pays a 1-Energy toll (1 of any good if
  out of energy) to use it. Bays lost their rent and became owner-only, so the shared things are the
  ones with a price and the private thing has none.
- **2026-09-09, third pass (no play).** Special Projects (F1) cut entirely. Every D/E zone is 1
  tick, 2 only when paid with time. The reset energy now prints only on Energy, Rock and Water.

## Setup

- Board fully open. All markers on their silo's bottom tick. Every bay square empty.
...
- **Silo board** (`/regolith/print/board`): three portrait letter sheets of two tall silos each —
  A/B1, B2/C1, C2/C3 — and two landscape sheets for the single-zone silos: D1/D2/D3 and E1/E2, two
  across with an odd last box centred. Every box is a tarot card on its side (4.75 : 2.75): portrait
  boxes are about 3.9 × 2.2in, landscape ones 5.1 × 3.0in. There is no sheet header, and every box
  has room for a meeple. A box reads left to right: the cost as resource tiles, a light dividing
  line, the yield as tiles (or an effect label), then a column of what can be owned on the zone. The
  tick track is the dividing line itself — one 16mm circle per tick, stacked up from the box's
  bottom edge and centred on the line, so the marker sits between cost and yield. The marker starts
  on zone I's bottom tick; there is no separate start cell. The zone's name is a small grey label
  top-left. A zone payable with time carries a quarter time-disc in its bottom-left corner with a −1
  badge. The bay is a 22mm grey square at the box's top-right, labelled, big enough for an owner's
  marker and a meeple. Nothing about the toll is printed: it is one rule for every slot. Below it, B
  zones print two 13mm machinery slots and C zones two polymer slots; other silos print neither.
  Undefined zones print as dashed boxes to pencil in.
- **Resource tiles:** every good on the board is a 12mm square tile, the good's mark filling it,
  ...
  A worker yield is not boxed — it is a piece, not a cube — but a 16mm worker figure standing on a
  short ground line, with a larger goods badge riding its shoulder: "+1" for Recruit, an up-chevron
  for Specialist. Machine, polymer and bay yields are still words until they have marks.
...
- **Player aid** (`/regolith/print/aid`): the three turn shapes, place / advance / bump / lock /
  reset (and its energy), paying with time, bays, the goods value strip, upgrade effects, and blanks
  for the setup values still to be tuned.
- **Bring:** meeples for workers plus a distinct piece per player for specialists, one 16mm disc per
  ...
  — Energy IV pays 16), and small markers per player for bay, machinery and polymer claims.
